...
output                                                     ssoe_o,
output                                                     lsbfe_o,
output       [`SPI_SAMPLE_O_WIDTH-1:0]                     sample_o,
output                                                     modfen_o,
output                                                     bidiroe_o,
output                                                     spiswai_o,
output                                                     spc0_o,
...
reg  [`SPI_REG_WIDTH-1:0] command_register_reg;
...
assign addr_valid = ((paddr_i == `SPI_CONTROL_REGISTER1_ADDR) ||
                    ...
                    (paddr_i == `SPI_COMMAND_REGISTER_ADDR));
...
begin
   ...
   begin
      ...
      command_register_reg <= `SPI_COMMAND_REGISTER_RST;
   end
   
   else
   begin
      if (wr_enb && (paddr_i == `SPI_CONTROL_REGISTER1_ADDR))
      begin
         control_register1_reg <= pwdata_i & `SPI_CONTROL_REGISTER1_MASK;
      end

      else if (wr_enb && (paddr_i == `SPI_CONTROL_REGISTER2_ADDR))
      begin
         control_register2_reg <= pwdata_i & `SPI_CONTROL_REGISTER2_MASK;
      end

      else if (wr_enb && (paddr_i == `SPI_BAUD_RATE_REGISTER_ADDR))
      begin
         baud_rate_register_reg <= pwdata_i & `SPI_BAUD_RATE_REGISTER_MASK;
      end

      else if (wr_enb && (paddr_i == `SPI_DATA_REGISTER_ADDR))
      begin
         data_register_reg <= pwdata_i & `SPI_DATA_REGISTER_MASK;
      end

      else if (wr_enb && (paddr_i == `SPI_COMMAND_REGISTER_ADDR))
      begin
         command_register_reg <= pwdata_i & `SPI_COMMAND_REGISTER_MASK;
      end
...
begin
   ...
   begin
      prdata_o = control_register1_reg & `SPI_CONTROL_REGISTER1_MASK;
   end

   else if (rd_enb && (paddr_i == `SPI_CONTROL_REGISTER2_ADDR))
   begin
      prdata_o = control_register2_reg & `SPI_CONTROL_REGISTER2_MASK;
   end

   else if (rd_enb && (paddr_i == `SPI_BAUD_RATE_REGISTER_ADDR))
   begin
      prdata_o = baud_rate_register_reg & `SPI_BAUD_RATE_REGISTER_MASK;
   end

   else if (rd_enb && (paddr_i == `SPI_STATUS_REGISTER_ADDR))
   begin
      prdata_o = status_register_reg & `SPI_STATUS_REGISTER_MASK;
   end

   else if (rd_enb && (paddr_i == `SPI_DATA_REGISTER_ADDR))
   begin
      prdata_o = data_register_reg & `SPI_DATA_REGISTER_MASK;
   end
...
assign sample_o = {control_register2_reg[`SPI_CONTROL_REGISTER2_SAMPLE2_POS], control_register2_reg[`SPI_CONTROL_REGISTER2_SAMPLE1_POS], control_register2_reg[`SPI_CONTROL_REGISTER2_SAMPLE0_POS]};
...
assign start_o = command_register_reg[`SPI_COMMAND_REGISTER_START_POS];
assign clear_o = command_register_reg[`SPI_COMMAND_REGISTER_CLEAR_POS];
assign abort_o = command_register_reg[`SPI_COMMAND_REGISTER_ABORT_POS];
